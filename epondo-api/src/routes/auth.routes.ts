...
export async function authRoutes(app: FastifyInstance) {
  // GET /api/auth/login - Redirect to eGovPH SSO (simplified for hackathon)
  app.get('/api/auth/login', async (request, reply) => {
    // In production, this would redirect to eGovPH's OAuth2 authorization URL.
    // For hackathon, the frontend handles the SSO redirect and sends us the exchange_code.
    return reply.send({
      success: true,
      data: { message: 'Use POST /api/auth/callback with exchange_code from eGovPH SSO' },
    });
  });

  // POST /api/auth/callback - Exchange code for user session
  app.post('/api/auth/callback', async (request, reply) => {
    ...
    try {
      ...
      if (existingUser) {
        ...
        // Determine role (in production, this would be assigned by admin or inferred)
